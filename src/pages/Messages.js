...
export default function Messages() {
  const [chatCtl, setChatCtl] = useState(new ChatController({ showDateTime: true }));
  ...
  useMemo(async () => {
    ...
    return () => setChatCtl(null);
  }, [chatCtl, activeUser.id]);

  const handleClick = (userId) => {
    setActiveUser(CHATLIST.find((u) => u.id === userId));
    const newMessages = MESSAGES.filter((m) => m.userId === userId);
    chatCtl.clearMessages();
    chatCtl.setMessages(newMessages.slice(0, -1));
    chatCtl.addMessage([...newMessages].pop());
  };
